    ...
    public partial class ProcedureCollectionView : XtraUserControl {
        ...
        void InitializeBindings() {
            ...
            fluentAPI.SetTrigger(x => x.EnabledTypes, ts => {
                tbNavPageAllProcedures.Enabled = ts.Contains(ProcedureType.Tooth) && ts.Contains(ProcedureType.General);
                tbNavPageGeneralProcedures.Enabled = ts.Contains(ProcedureType.General);
                tbNavPageToothProcedures.Enabled = ts.Contains(ProcedureType.Tooth);
                if(proceduresTabPane.SelectedPage == null || !proceduresTabPane.SelectedPage.Enabled)
                    proceduresTabPane.SelectedPage = proceduresTabPane.Pages.FirstOrDefault(x => x.Enabled) as TabNavigationPage;
            });
